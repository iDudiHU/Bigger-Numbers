...
public class UIManager : MonoBehaviour
{
    ...
    private bool isAttributeSpent = false;
    /// <summary>
    /// Description:
    ...
    {
            if (isUpgrading && isAttributeSpent)
            {
                CursorManager.instance.ChangeCursorMode(CursorManager.CursorState.FPS);
                GoToPageByName("InGamePage");
                Time.timeScale = 1;
                isUpgrading = false;
                isAttributeSpent = false;
            }
            else
            {
                CursorManager.instance.ChangeCursorMode(CursorManager.CursorState.Menu);
    ...
    void SubscriebeToLevelUp()
	{
        levelSystem.OnLevelChanged += LevelSystem_OnLevelChanged;
		levelSystem.OnAllAttributesSpent += LevelSystem_OnAllAttributesSpent;
		levelSystem.OnAttributeSpent += LevelSystem_OnAttributeSpent;
	}

	private void LevelSystem_OnAllAttributesSpent(object sender, System.EventArgs e)
	{
		ToggleLevelUp();
	}
    ...
    private void LevelSystem_OnAttributeSpent(object sender, System.EventArgs e)
    {
        isAttributeSpent = true;
    }
}
